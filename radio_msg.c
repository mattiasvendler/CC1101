...
void radio_msg_hello_init(u8_t *buf,u32_t my_address) {
	...
	seq++;
	hello->header.target = 0xFFFFFFFF;
	hello->header.source = my_address;
...
void radio_msg_login_response_init(u8_t *buf, u32_t target, u32_t my_address) {
	...
	seq++;
	login_response->header.target = target;
	login_response->header.source = my_address;
	login_response->header.seq = seq;
